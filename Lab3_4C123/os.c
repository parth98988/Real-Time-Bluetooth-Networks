...
// ****IMPLEMENT THIS****
	...
	if(Index < NUMPERIODIC){
		eventPeriod[Index].Periodic_TaskPt = thread;
		eventPeriod[Index].period = period;
		Index++;
	}
  return 1;
}
...
maxcount = 1;
	for(i=0;i<NUMPERIODIC;i++){
		maxcount = maxcount*eventPeriod[i].period;
	}
	if ((Counter%(eventPeriod[0].period))== 0){
				(*(eventPeriod[0].Periodic_TaskPt))();
}
	if ((Counter%(eventPeriod[1].period))== 1){
				(*(eventPeriod[1].Periodic_TaskPt))();
}
...
void OS_Sleep(uint32_t sleepTime){uint32_t tskPeriod;
